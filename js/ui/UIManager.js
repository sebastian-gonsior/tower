...
export class UIManager {
    ...
    /**
     * Render active debuffs near health bars.
     * Shows debuff icons with stack counts, total damage values, and remaining duration for: bleed, poison, fire, shadow, curse, frozen, holy
     */
    renderDebuffs() {
        const debuffIcons = {
            bleed: '🩸',
        ...
        const renderDebuffContainer = (container, debuffs) => {
            ...
            // Group debuffs by type, count stacks, sum damage values, and track max duration
            const debuffData = {};
            debuffs.forEach(debuff => {
                if (!debuffData[debuff.type]) {
                    debuffData[debuff.type] = { count: 0, totalDamage: 0, maxDuration: 0 };
                }
                debuffData[debuff.type].count++;
                debuffData[debuff.type].totalDamage += debuff.damagePerTick || 0;
                // Track the maximum remaining duration (since debuffs refresh, show the highest)
                if (debuff.duration > debuffData[debuff.type].maxDuration) {
                    debuffData[debuff.type].maxDuration = debuff.duration;
                }
            });
            
            // Render each debuff type with stack count, value, and duration
            for (const [type, data] of Object.entries(debuffData)) {
                ...
                // Damage value badge (top-left) - ALWAYS visible
                const valueSpan = document.createElement('span');
                valueSpan.className = 'debuff-value';
                valueSpan.innerText = data.totalDamage > 0 ? data.totalDamage : (type === 'frozen' ? '❄' : '0');
                debuffEl.appendChild(valueSpan);
                
                // Duration badge (top-right) - ALWAYS visible
     ...
     * Shows buff icons with values for: speedBonus, critChance, multihit
     */
    renderBuffs() {
        const buffIcons = {
            speedBonus: '⚡',
